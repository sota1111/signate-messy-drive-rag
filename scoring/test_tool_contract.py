...
def test_caption_figure_literal_filter_rejects_adjacent_cell_item(monkeypatch):
    pdf = next((r for r in walk() if r.ext == "pdf"), None)
    if pdf is None:
        pytest.skip("no PDF in corpus")
    monkeypatch.setattr(
        _vision, "pdf_page_images", lambda path: [(b"page", "image/jpeg")])

    def fake_generate(prompt, *, images, **kwargs):
        return json.dumps({"matches": [
            {
                "page": 1, "scope": "role", "candidate": "モデル再現コード提供",
                "condition": "別契約", "source": "モデル再現コード提供、監視ダッシュボード構築（別契約）",
                "conditioned_text": "モデル再現コード提供", "same_visual_line": False,
            },
            {
                "page": 1, "scope": "role", "candidate": "監視ダッシュボード構築",
                "condition": "別契約", "source": "モデル再現コード提供、監視ダッシュボード構築（別契約）",
                "conditioned_text": "監視ダッシュボード構築（別契約）", "same_visual_line": True,
            },
        ]}, ensure_ascii=False)

    monkeypatch.setattr("src.rag.llm.generate", fake_generate)
    out = caption_figure(pdf, question="『別契約』と明記された役務は何ですか。")
    assert [item["candidate"] for item in out["value"]] == ["監視ダッシュボード構築"]
