#!/usr/bin/env bash
# SOT-2709 (cycle11) — Sheet2 入れ子(compact nested)ピボットの row_context 再構成 focused gate
# (official:false lane). RAG_HIGHLIGHT_PIVOT_CONTEXT ビルダーの入れ子対応(carry-down + 値域レベル命名)。
#
...
# Target: idx77（蒼泉会 ひがし丘総合病院 train.xlsx Sheet2 の黄セル E14 →
#   gold=「children=3、smoker=no で抽出されたデータに対する合計 / age」）+ guard idx42（回帰なし）。
#   D列=行ラベル総称ヘッダの compact 入れ子: 外側 children を内側 smoker にキャリーダウンし、
#   値域照合(train シート)でレベル名を解決。gold 参照なし・ハードコードなし。
#
...
# rebuilt WITH RAG_HIGHLIGHT_PIVOT_CONTEXT=1 so the SOT-2709 pivot context is baked into visual_store.jsonl
...
export RAG_MCP_TOOL_LOG=artifacts/sot2709_tool_calls.jsonl
export RAG_CLAUDE_MCP_RESUME=0

TARGET="${TARGET:-77,42}"
GOLD="${GOLD:-artifacts/predictions_test_v4_final.csv}"

echo "=== SOT-2709 cycle11 nested-pivot focused gate start $(date -u +%FT%TZ) ==="
...
.venv/bin/python scripts/run_focused_gate.py \
  ...
  --dev --no-smoke \
  --workers 1
echo "=== SOT-2709 cycle11 nested-pivot focused gate done $(date -u +%FT%TZ) exit=$? ==="
