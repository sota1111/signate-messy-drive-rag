#!/usr/bin/env bash
# SOT-2696 — Sonnet gold100 improvement cycle 9, integrated config (official:false lane).
#
# Base env = scripts/sonnet_gold_cycle8.sh (net84 実証済み: cycle8 gold100 = 89 match / 6 abstain /
# 5 wrong) PLUS the cycle-9 child levers that PASSED their focused gates (all four children):
...
#   (C4 SOT-2700) RAG_VDIFF_STRUCT=1 — diffpair/diff_store の決定論構造修正 (idx14/95 回復;
#                 idx1/22 は plan_fanout LLM 逐語脱落が残余 = direct-commit 昇格は次サイクル軸)
# Composition was verified by scripts/sonnet_gold_cycle9_integrated_focused.sh (sentinel gate,
...
# indexes), so replaying the cycle-8 sidecar would mask the change on every LLM lane. The fresh
...
export RAG_CLAUDE_MCP_RESUME=artifacts/gold100_sonnet_cycle9_resume.jsonl
export RAG_MCP_TOOL_LOG=artifacts/gold100_sonnet_cycle9_tool_calls.jsonl

GOLD="${GOLD:-artifacts/predictions_test_v4_final.csv}"

echo "=== SOT-2696 Sonnet gold100 cycle9 (official:false) start $(date -u +%FT%TZ) ==="
...
.venv/bin/python -m scoring.gold_offline --run --workers 1 --no-official \
  --gold "$GOLD" \
  --out artifacts/gold100_sonnet_cycle9.json
echo "=== SOT-2696 Sonnet gold100 cycle9 done $(date -u +%FT%TZ) exit=$? ==="
