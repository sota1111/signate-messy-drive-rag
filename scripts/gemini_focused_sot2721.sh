#!/usr/bin/env bash
# SOT-2721 — backend=gemini FOCUSED verification (no submit, no champion touch).
# Cheap pre-check on the Gemini flash-3.6 serve path for idx29 (TPヒストグラム3番目ビンの決定論direct-commit)
# recovery target + the derived-numeric regression guard (idx17/30/33/36/54/57/63/83/99) + 10 sentinels,
...
# --- SOT-2720 レバーA: 小数第N位指定問の決定論丸め (idx57 0.42395962->0.42396 / idx63 0.15001822->0.15002) ---
export RAG_DECIMAL_PRECISION=1
# --- SOT-2720 レバーB: Gemini経路の該当なし裸形式 fold (idx85「未達成…はありません（全6項目達成）」->「該当なし」) ---
export RAG_NONE_BARE_FOLD=1
# --- SOT-2721 lever: TPヒストグラムのN番目ビン範囲を決定論再集計でdirect-commit (idx29「わかりません」->「6.088138 ~ 6.288138」) ---
export RAG_HIST_BIN=1
export RAG_INVESTIGATOR_BACKEND=gemini
OUT="${FOCUSED_OUT:-artifacts/focused_gemini_sot2721.json}"
# target = idx29 (TPヒストグラム3番目ビン範囲の決定論回収) + 回帰ガード idx17/30/33/36/54/57/63/83/99 (他 derived 数値不変)
.venv/bin/python scripts/run_focused_gate.py \
  --label sot2721-histogram-bin-gemini --issue SOT-2721 \
  --target "29,17,30,33,36,54,57,63,83,99" \
