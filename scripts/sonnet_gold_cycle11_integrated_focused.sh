#!/usr/bin/env bash
# SOT-2708 — cycle-11 INTEGRATED focused gate (official:false lane).
#
...
# Targets: C1 77 / C2 2,41,75,89,90 / C3 3,11,49 / C4 9,14 / C5 13,43,46 / C6 27
#          + guards 16,21,22,31,42,71,87,95. Union = 23 idx.
# idx1 は SOT-2715 honest 撤退（serve byte-identical）につき対象外。
#
...
# (SOT-2664 gotcha) — and the cycle-11 levers change answers globally, so any sidecar replay would
...
export RAG_MCP_TOOL_LOG=artifacts/sonnet_cycle11_integrated_tool_calls.jsonl
export RAG_CLAUDE_MCP_RESUME=0

TARGET="${TARGET:-2,3,9,11,13,14,16,21,22,27,31,41,42,43,46,49,71,75,77,87,89,90,95}"
GOLD="${GOLD:-artifacts/predictions_test_v4_final.csv}"

echo "=== SOT-2708 cycle11 integrated focused gate start $(date -u +%FT%TZ) ==="
echo "BACKEND=$RAG_INVESTIGATOR_BACKEND SCHEDULE_PLAN=$RAG_SCHEDULE_PLAN_LOOKUP DOCX_COMMENT=$RAG_DOCX_COMMENT_ANCHOR VDIFF_DC_NOCHANGE=$RAG_VDIFF_DC_NOCHANGE VDIFF_DC_COLRENAME=$RAG_VDIFF_DC_COLRENAME MASTER_JOIN=$RAG_MASTER_JOIN_LOOKUP PPTX_NOTE_SCOPE=$RAG_PPTX_NOTE_SCOPE RESUME=$RAG_CLAUDE_MCP_RESUME TARGET=$TARGET GOLD=$GOLD"
.venv/bin/python scripts/run_focused_gate.py \
  --label sot2708_cycle11_integrated \
  --target "$TARGET" \
  ...
  --dev --no-smoke \
  --workers 1
echo "=== SOT-2708 cycle11 integrated focused gate done $(date -u +%FT%TZ) exit=$? ==="
