#!/usr/bin/env bash
# SOT-2679 — focused gate for cycle6 K3 (派生メトリクス網羅拡張, official:false).
#
# Base = scripts/sonnet_gold_cycle5.sh (net59 実証構成) PLUS the one K3 lever under test:
#   RAG_DERIVED_COVERAGE=1 — 派生メトリクスストアの網羅拡張を serve へ配線する決定論直答レーン(+補助ツール):
#     (idx4) 目的変数と相関が最も高い数値特徴量 = correlations.with_target の |r| 単独最大(id 列除外)。
#     (idx24) 1つでも欠損値がある行数が最も多い案件(主略称) = 全案件 canonical train 表・全列走査の欠損
#             行数(独立検算)の厳密単独最大。SOT-2663 で余剰予算 ad-hoc 列挙が誤案件『白峰』を確定させた
#             逆流に対し、ストア到達(canonical 全案件の決定論比較)のみが安全経路。full-coverage かつ単独
#             最大でない時は棄権(idx24 ガード: Missing→MATCH のみ許容, Incorrect を作らない)。serve gated・既定 OFF。
# Targets (gold=artifacts/predictions_test_v3_final.csv): 一次 idx4/24, 二次 idx47/83.
...
# derived_metrics is rebuilt because the new lane reads its correlations.with_target / missing blocks.
...
# --- axis under test (SOT-2679, cycle6 K3) ---
...
export RAG_MCP_TOOL_LOG=artifacts/sonnet_child_derived_coverage_tool_calls.jsonl

TARGET="${TARGET:-4,24,47,83}"

echo "=== SOT-2679 Sonnet K3 derived-coverage focused gate start $(date -u +%FT%TZ) ==="
echo "BACKEND=$RAG_INVESTIGATOR_BACKEND DERIVED_COVERAGE=$RAG_DERIVED_COVERAGE RESUME=$RAG_CLAUDE_MCP_RESUME TARGET=$TARGET"
.venv/bin/python scripts/run_focused_gate.py \
  --label sot_child_derived_coverage \
  --target "$TARGET" \
  --sentinels scripts/sonnet_sentinels.json \
  --issue SOT-2679 \
  --dev --no-smoke \
  --workers 1
echo "=== SOT-2679 Sonnet K3 derived-coverage focused gate done $(date -u +%FT%TZ) exit=$? ==="
