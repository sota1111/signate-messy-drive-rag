#!/usr/bin/env bash
# SOT-2707 (cycle10) — 契約書署名欄を権威ソースとする担当者マスタ 完全ラベルレーン focused gate
# (official:false lane).
#
# Base env = scripts/sonnet_gold_cycle9.sh のフラグ一式（net84+ 実証済み）＋本子の新フラグ:
#   RAG_CONTACT_MASTER=1 — 契約書署名欄/当事者ブロックの 会社名/部署名/主担当者/役職(甲=client・乙=vendor)
#                          を質問非依存に全数抽出し、クライアント主担当者の役職を裸形式で直答するレーン。
# Target: idx21（青葉バイオメディカル機器のクライアント主担当者の役職 → gold=人材戦略部長）。cycle9 実測は
#   会議録の出席者展開形『人事本部 人材戦略部 部長』を写経して Incorrect。契約書署名欄の構造化フィールド
#   『役職：人材戦略部長』が権威ソース（SOT-2666 完全ラベル写経の権威ソース選好版）。
#
# 番兵: scripts/sonnet_sentinels.json (idx58 版)。Gate FAILS on any sentinel regression (10/10)。
# GOLD = artifacts/predictions_test_v4_final.csv。gold ハードコードなし（store は原文ラベルの完全写経のみ）。
...
export RAG_MCP_TOOL_LOG=artifacts/sot2707_tool_calls.jsonl
export RAG_CLAUDE_MCP_RESUME=0

TARGET="${TARGET:-21}"
GOLD="${GOLD:-artifacts/predictions_test_v4_final.csv}"

echo "=== SOT-2707 cycle10 contact-master focused gate start $(date -u +%FT%TZ) ==="
...
.venv/bin/python scripts/run_focused_gate.py \
  ...
  --dev --no-smoke \
  --workers 1
echo "=== SOT-2707 cycle10 contact-master focused gate done $(date -u +%FT%TZ) exit=$? ==="
