#!/usr/bin/env bash
# SOT-2649 — Sonnet gold100 improvement cycle 2 (official:false lane).
#
# Base env = SOT-2628 Sonnet dev gold100 (champion Wave A + B1, investigator on flat-rate Sonnet via
# RAG_INVESTIGATOR_BACKEND=claude-mcp) with the SAME THREE levers as cycle 1 on top. The cycle-2
# delta is IN-CODE: SOT-2649 deterministic-lane coverage for idx38 (empty-composite enum) and idx57
# (shared-stem metric-presence bind) — both were tool-choice-dependent under Sonnet in cycle 1.
...
#   2. RAG_FORBID_GEMINI=1 — no-Gemini guard (SOT-2649): any genai client access during this run
...
# --- cycle-2 levers (same lever set as cycle 1; delta is in-code lane coverage, see header) ---
...
export RAG_CLAUDE_MCP_RESUME=artifacts/gold100_sonnet_cycle2_resume.jsonl
export RAG_MCP_TOOL_LOG=artifacts/gold100_sonnet_cycle2_tool_calls.jsonl

echo "=== SOT-2649 Sonnet gold100 cycle2 (official:false) start $(date -u +%FT%TZ) ==="
...
.venv/bin/python -m scoring.gold_offline --run --workers 1 --no-official \
  --out artifacts/gold100_sonnet_cycle2.json
echo "=== SOT-2649 Sonnet gold100 cycle2 done $(date -u +%FT%TZ) exit=$? ==="
