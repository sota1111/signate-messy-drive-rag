#!/usr/bin/env bash
# SOT-2701 — cycle-10 INTEGRATED focused gate (official:false lane).
#
# Env = scripts/sonnet_gold_cycle9.sh (net86 実証済み: cycle9 gold100 = 90 match / 6 abstain /
# 4 wrong) + ALL promoted cycle-10 child levers combined:
...
#   (C3 SOT-2704) RAG_HIGHLIGHT_PIVOT_CONTEXT=1 — xlsx ハイライトのピボット文脈焼き込み (idx42)
#   (C4 SOT-2705) RAG_PPTX_MONEY_PAGE=1 — pptx 金額提示ページストア＋決定論レーン (idx59)
#   (C5 SOT-2706) RAG_VDIFF_DIRECT_COMMIT=1 — vdiff record summary direct-commit 昇格 (idx22;
#                 idx1 は意味枠語彙が導出不能で honest 残余)
#   (C6 SOT-2707) RAG_CONTACT_MASTER=1 — 契約書署名欄 contact-master 役職レーン (idx21)
...
#
# Targets: C1 52 / C2 16,71 / C3 42 / C4 59 / C5 1,22 / C6 21 + guards 9,14,95. Union = 11 idx.
...
# (SOT-2664 gotcha) — and the cycle-10 levers change answers globally, so any sidecar replay would
...
export RAG_MCP_TOOL_LOG=artifacts/sonnet_cycle10_integrated_tool_calls.jsonl
export RAG_CLAUDE_MCP_RESUME=0

TARGET="${TARGET:-1,9,14,16,21,22,42,52,59,71,95}"
GOLD="${GOLD:-artifacts/predictions_test_v4_final.csv}"

echo "=== SOT-2701 cycle10 integrated focused gate start $(date -u +%FT%TZ) ==="
echo "BACKEND=$RAG_INVESTIGATOR_BACKEND SEP_CONTRACT_ROLE=$RAG_SEP_CONTRACT_ROLE FORMAT_FACTS=$RAG_FORMAT_FACTS HIGHLIGHT_PIVOT=$RAG_HIGHLIGHT_PIVOT_CONTEXT PPTX_MONEY_PAGE=$RAG_PPTX_MONEY_PAGE VDIFF_DIRECT_COMMIT=$RAG_VDIFF_DIRECT_COMMIT CONTACT_MASTER=$RAG_CONTACT_MASTER RESUME=$RAG_CLAUDE_MCP_RESUME TARGET=$TARGET GOLD=$GOLD"
.venv/bin/python scripts/run_focused_gate.py \
  --label sot2701_cycle10_integrated \
  --target "$TARGET" \
  ...
  --dev --no-smoke \
  --workers 1
echo "=== SOT-2701 cycle10 integrated focused gate done $(date -u +%FT%TZ) exit=$? ==="
