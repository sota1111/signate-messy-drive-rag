#!/usr/bin/env bash
# SOT-2677 — focused gate for cycle6 K1 (文書テーブル/全文チャンクストア, official:false).
#
# Base = scripts/sonnet_gold_cycle5.sh (net59 実証構成) PLUS the one K1 lever under test:
#   RAG_DOC_REACH_STORE=1 — 全 docx/pptx/pdf の (a) 全表→行列 table store と (b) 切り詰めなし全文チャンク
#     store を serve へ配線する lookup ツール 2 本(doc_table_lookup / doc_fulltext_search):
#     read_office は investigator の 8000 字境界(_jsonable max_str=8000)で末尾を切り詰めるため、長文 docx の
#     末尾表/後半記述へ到達できず予算切れ棄権していた(idx8/50 東都=15888字, idx99 みなみ野=9191字)。
#     table lookup は表を直接返し、fulltext search は keyword 近傍窓/末尾窓で 8000 字の向こう側へ到達させる。
#     serve gated・既定 OFF ⇒ OFF 時 byte-identical。
# Targets (gold=artifacts/predictions_test_v3_final.csv): 一次 idx8/50/99, 二次 idx17/48/52/53/68/71.
...
# doc_reach_store is (re)built because the new lookup tools read its tables / full_text.
...
# --- axis under test (SOT-2677, cycle6 K1) ---
...
export RAG_MCP_TOOL_LOG=artifacts/sonnet_child_doc_reach_tool_calls.jsonl

TARGET="${TARGET:-8,50,99,17,48,52,53,68,71}"

echo "=== SOT-2677 Sonnet K1 doc-reach focused gate start $(date -u +%FT%TZ) ==="
echo "BACKEND=$RAG_INVESTIGATOR_BACKEND DOC_REACH_STORE=$RAG_DOC_REACH_STORE RESUME=$RAG_CLAUDE_MCP_RESUME TARGET=$TARGET"
.venv/bin/python scripts/run_focused_gate.py \
  --label sot_child_doc_reach \
  --target "$TARGET" \
  --sentinels scripts/sonnet_sentinels.json \
  --issue SOT-2677 \
  --dev --no-smoke \
  --workers 1
echo "=== SOT-2677 Sonnet K1 doc-reach focused gate done $(date -u +%FT%TZ) exit=$? ==="
