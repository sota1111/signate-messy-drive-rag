#!/usr/bin/env bash
# SOT-2718 — backend=gemini FOCUSED verification (official:false/dev; no submit, no champion touch).
# Cheap pre-check on the Gemini flash-3.6 serve path for the idx8/52 recovery targets + 10 sentinels,
...
OUT="${FOCUSED_OUT:-artifacts/focused_gemini_sot2718.json}"
# target = levers idx1,8 + ALL version_diff idx (0/9/14/22/74/95 regression guard) + idx52 (SOT-2717 recovery holds)
.venv/bin/python scripts/run_focused_gate.py \
  --label sot2718-vdiff-lowconf-idx8-gemini --issue SOT-2718 \
  --target "1,8,0,9,14,22,74,95,52" \
