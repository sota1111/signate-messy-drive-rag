#!/usr/bin/env bash
# SOT-2714 (cycle11) — pptx 発表者ノート「スコープ対象外」✖項目 決定論カウントレーン focused gate
# (official:false lane).
#
# Base env = cycle9 champion のフラグ一式＋本子の新フラグ:
#   RAG_PPTX_NOTE_SCOPE=1 — pptx_note_scope_store が build 時に、全 pptx の発表者ノートを質問非依存で走査し、
#     見出し「スコープ対象外」の直下に連続する ✖ 段落を1項目として数えて {scope_excluded_count, items[]} で
#     全数焼き、serve の決定論レーン (fact_layer 下位) が「◯◯の提案書でスコープ対象外はいくつ」型質問へ、
#     対象 pptx を案件 + doc-kind『提案書』で一意束縛でき count>0 の時だけ裸整数を直答する（曖昧なら従来経路へ
#     defer）。gold 値（7）ハードコードなし・段落単位カウントで導出。
# Target: idx27（恒一会 かえで総合病院 提案書.pptx notesSlide1『スコープ対象外』直下 ✖段落 7本 → gold=7）。
#   原 pptx の OOXML notesSlide のみから決定論導出（gold 参照なし・ハードコードなし）。
#
...
export RAG_MCP_TOOL_LOG=artifacts/sot2714_tool_calls.jsonl
export RAG_CLAUDE_MCP_RESUME=0

TARGET="${TARGET:-27}"
GOLD="${GOLD:-artifacts/predictions_test_v4_final.csv}"

echo "=== SOT-2714 cycle11 pptx-note-scope focused gate start $(date -u +%FT%TZ) ==="
echo "BACKEND=$RAG_INVESTIGATOR_BACKEND PPTX_NOTE_SCOPE=$RAG_PPTX_NOTE_SCOPE RESUME=$RAG_CLAUDE_MCP_RESUME TARGET=$TARGET GOLD=$GOLD"
.venv/bin/python scripts/run_focused_gate.py \
  --label sot2714_pptx_note_scope \
  --target "$TARGET" \
  ...
  --dev --no-smoke \
  --workers 1
echo "=== SOT-2714 cycle11 pptx-note-scope focused gate done $(date -u +%FT%TZ) exit=$? ==="
