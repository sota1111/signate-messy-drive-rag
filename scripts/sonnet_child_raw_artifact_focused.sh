#!/usr/bin/env bash
# SOT-2678 — focused gate for cycle6 K2 (分析成果物 raw ファイルストア, official:false).
#
# Base = scripts/sonnet_gold_cycle5.sh (net59 実証構成) PLUS the one K2 lever under test:
#   RAG_RAW_ARTIFACT_STORE=1 — 全案件×全成果物(.py/.json/.csv/.toml/.ipynb/設定)を質問非依存 ingest した
#     raw-artifact store を serve へ配線する lookup ツール 2 本(artifact_grep / analysis_artifact_lookup):
#     .py は read_office 非対応・metrics.json/leaderboard.csv は find_files 0件で investigator が到達できず
#     予算切れ棄権していた(idx32 青嶺 features.py/metrics.json, idx61 京橋 modeling.py の適用ハイパラ,
#     idx62 青葉与信 leaderboard.csv の上位2件設定差分)。artifact_grep はコード/設定/CSV の中身へ行番号付きで
#     到達し、analysis_artifact_lookup は config.model_params＋コード上デフォルトをマージした適用ハイパラと
#     leaderboard 最良順行を構造化して返す。DB経路(RAW_FILE_TOOLS ではない, SOT-2660)。serve gated・既定 OFF
#     ⇒ OFF 時 byte-identical。
# Targets (gold=artifacts/predictions_test_v3_final.csv): 一次 idx32/61/62, 二次 idx35/36/73.
...
# raw_artifact_store is (re)built because the new lookup tools read its per-file text / per-case rollup.
...
# --- axis under test (SOT-2678, cycle6 K2) ---
...
export RAG_MCP_TOOL_LOG=artifacts/sonnet_child_raw_artifact_tool_calls.jsonl

TARGET="${TARGET:-32,61,62,35,36,73}"

echo "=== SOT-2678 Sonnet K2 raw-artifact focused gate start $(date -u +%FT%TZ) ==="
echo "BACKEND=$RAG_INVESTIGATOR_BACKEND RAW_ARTIFACT_STORE=$RAG_RAW_ARTIFACT_STORE RESUME=$RAG_CLAUDE_MCP_RESUME TARGET=$TARGET"
.venv/bin/python scripts/run_focused_gate.py \
  --label sot_child_raw_artifact \
  --target "$TARGET" \
  --sentinels scripts/sonnet_sentinels.json \
  --issue SOT-2678 \
  --dev --no-smoke \
  --workers 1
echo "=== SOT-2678 Sonnet K2 raw-artifact focused gate done $(date -u +%FT%TZ) exit=$? ==="
