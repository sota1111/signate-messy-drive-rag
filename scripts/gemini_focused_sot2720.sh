#!/usr/bin/env bash
# SOT-2720 — backend=gemini FOCUSED verification (official:false/dev; no submit, no champion touch).
# Cheap pre-check on the Gemini flash-3.6 serve path for idx57/63 (小数第N位 決定論丸め) + idx85 (該当なし裸形式)
# recovery targets + the 小数精度/該当なし regression guard (idx17/30/33/36/54/83/99/9/38) + 10 sentinels,
...
OUT="${FOCUSED_OUT:-artifacts/focused_gemini_sot2720.json}"
# target = idx57/63 (小数第5位丸め回収) + idx85 (該当なし裸形式回収) + 回帰ガード idx17/30/33/54/83/99 (指定桁不変)・
# idx36 (17桁 full precision 不変・小数第N位無し)・idx9/38 (既存の該当なし維持)
.venv/bin/python scripts/run_focused_gate.py \
  --label sot2720-decimal-precision-none-bare-gemini --issue SOT-2720 \
  --target "57,63,85,17,30,33,36,54,83,99,9,38" \
