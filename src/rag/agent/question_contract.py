...
_NO_SPECIAL_REGULATION_RE = re.compile(
    ...
    # SOT-2548: a "not found" phrasing of the same negative finding — e.g. idx78's
    # 「ACTHに関する記述が見つかりませんでした」 — must trip the completeness guard just like 「存在しない」.
    r"(?:規定|規約|契約条件|条項|記載|記述|定め|該当).{0,20}"
    r"(?:見つか(?:らない|りません|らず|らなかった)|見当たら(?:ない|ず|ません|なかった)|"
    r"確認でき(?:ない|ず|ません|なかった))")
